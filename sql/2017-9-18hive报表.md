# hive报表 #
## 1.数据库的表结构 ##
从第三方网站中抓取的log日志数据（我们在今天启动服务，获取昨天的完整数据，因此获取的数据都是昨天的统计结果）经过ETL放入到events2表中，其中log日志中的主要内容放入到xcontext字段中，可以看出该字段的类型时一种map结构，类似于字段中嵌套着字段。
